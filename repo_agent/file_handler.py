# FileHandler class: handles read/write operations for markdown and Python files
...
class FileHandler:
    """
    Creates one instance per changed file in the loop that processes changed files.
    """

    def __init__(self, repo_path, file_path):
        self.file_path = file_path  # file_path is relative to the repository root
    ...
    def get_obj_code_info(
        ...
        with open(
            ...
            # Get the column position of the object name in the first line of code
            name_column = lines[start_line - 1].find(code_name)
            # Check whether the code contains a return statement
    ...
    def write_file(self, file_path, content):
        ...
        """
        # Ensure file_path is a relative path
    ...
    def get_end_lineno(self, node):
        ...
        if not hasattr(node, "lineno"):
            return -1  # Return -1 to indicate this node has no line number

        end_lineno = node.lineno
        for child in ast.iter_child_nodes(node):
            child_end = getattr(child, "end_lineno", None) or self.get_end_lineno(child)
            if child_end > -1:  # Only update when the child node has a valid line number
    ...
    def generate_overall_structure(self, file_path_reflections, jump_files) -> dict:
        """Get the file layout of the target repo via AST-walk to extract all objects.
        Files in jump_files are skipped and treated as non-existent.
        """
        repo_structure = {}
